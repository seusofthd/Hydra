...
import com.symlab.hydra.experiment.Logger;
...
public class Profiler {
	...
	private static final String logFileName = storage + "/l.txt";
	...
	private double estimatedBluetoothEnergy;
	private long estimatedRemoteEnergy;
	private final long constEnergy = 700;
	...
	public Profiler(Context context, ProgramProfiler progProfiler, DeviceProfiler devProfiler, BluetoothProfiler btProfiler) {
		this.progProfiler = progProfiler;
		this.devProfiler = devProfiler;
		//if (Constants.NETWORK_TYPE == Constants.BLUETOOTH)
		this.btProfiler = btProfiler;
...
*/
	...
	public LogRecord stopAndLogExecutionInfoTracking(boolean receivedTask) {
		...
		record.bluetoothEnergy = estimatedBluetoothEnergy;
	
		//Log.i(" Profiler", "Log record - " + record.toString());
		if (!receivedTask) {
			record.bluetoothEnergy = 2*estimatedBluetoothEnergy;
			try {
				if (logFileWriter == null) {
					File logFile = new File(logFileName);
					logFile.createNewFile(); // Try creating new, if doesn't exist
	...
	public static void recordTransmissionTime(long time) {
		try {
			if (logFileWriter == null) {
				File logFile = new File(logFileName);
				logFile.createNewFile(); // Try creating new, if doesn't exist
			...
			logFileWriter.append("Transmit object: " + (time/1000000) + "ms\r\n");
			logFileWriter.flush();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void updateDB()
	{
		 ...
		 */
        queryString = query.getData(new String[] {"execDuration", "energyConsumption", "recordQuantity"}, selection, 
	...
	public void estimateEnergyConsumption()
	{
		...
		System.out.println(devProfiler.screenBrightness);

		estimatedBluetoothEnergy = estimatedBluetoothEnergy();
		Log.d("PowerDroid-Energy", "Bluetooth energy: " + estimatedBluetoothEnergy + " mJ");
		
		estimatedRemoteEnergy = estimatedRemoteEnergy();
		Log.d("PowerDroid-Energy", "Remote energy: " + estimatedRemoteEnergy + " mJ");
		
		totalEstimatedEnergy = (long) (estimatedCpuEnergy + estimatedScreenEnergy + estimatedBluetoothEnergy) + estimatedRemoteEnergy;
	...
	private double estimateCpuEnergy(int duration)
	{
		...
		for(int i = 0; i < duration; i++)
		{
			...
			Logger.log("" + freqH + " " + freqL + " " + util + " " + cpuON + "\r\n");
	...
	private double estimateScreenEnergy(int duration)
	{
		...
		double betaBrightness = 3.0822;
		
		for(int i = 0; i < duration; i++) {
			if (devProfiler.getScreenBrightness(i) == -1) continue;
			estimatedScreenEnergy += betaBrightness * devProfiler.getScreenBrightness(i) + constEnergy;
		}
	...
	private double estimatedBluetoothEnergy() {
		double avgBTPower = 100f;
		return avgBTPower*btProfiler.totalOnTime()/1000000000f;
	}

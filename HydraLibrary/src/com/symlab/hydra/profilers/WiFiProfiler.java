...
public class WiFiProfiler {
	 ...
	 */
	/*
	 * public static int rttPing(InputStream in, OutputStream out) { Log.d(TAG,
	 * "Pinging"); int tRtt = 0; int response; try { for (int i = 0; i <
	 * rttPings; i++) { Long start = System.nanoTime(); Log.d(TAG, "Send Ping");
	 * out.write(Msg.NETWORK_PING); out.flush(); Log.d(TAG, "Read Response");
	 * response = in.read(); // response = (Integer)in.readObject(); if
	 * (response == Msg.NETWORK_PONG) tRtt = (int) (tRtt + (System.nanoTime() -
	 * start) / 2); else { Log.d(TAG, "Bad Response to Ping - " + response);
	 * tRtt = rttInfinite; }
	 * 
	 * } //out.write(Msg.NETWORK_CLOSE); //out.flush();
	 * 
	 * rtt = tRtt / rttPings; Log.d("PowerDroid-Profiler", "Ping - " + rtt /
	 * 1000000 + "ms");
	 * 
	 * } catch (IOException e) { // e.printStackTrace(); tRtt = rttInfinite; }
	 * return rtt; }
	 */
	private int uid;
	 ...
	 */
	/*
	 * public void registerNetworkStateTrackers() { networkStateReceiver = new
	 * BroadcastReceiver() { public void onReceive(Context context, Intent
	 * intent) { // context.unregisterReceiver(this);
	 * 
	 * netInfo = connectivityManager.getActiveNetworkInfo(); if (netInfo ==
	 * null) { Log.d("PowerDroid-Profiler", "No Connectivity");
	 * currentNetworkTypeName = ""; currentNetworkSubtypeName = ""; } else {
	 * Log.d("PowerDroid-Profiler", "Connected to network type " +
	 * netInfo.getTypeName() + " subtype " + netInfo.getSubtypeName());
	 ...
	 * 
	 * @Override public void onDataConnectionStateChanged(int state, int
	 * networkType) { if (state == TelephonyManager.DATA_CONNECTED) { if
	 * (networkType == TelephonyManager.NETWORK_TYPE_EDGE)
	 * Log.d("PowerDroid-Profiler", "Connected to EDGE network"); else if
	 * (networkType == TelephonyManager.NETWORK_TYPE_GPRS)
	 * Log.d("PowerDroid-Profiler", "Connected to GPRS network"); else if
	 * (networkType == TelephonyManager.NETWORK_TYPE_UMTS)
	 * Log.d("PowerDroid-Profiler", "Connected to UMTS network"); else
	 * Log.d("PowerDroid-Profiler", "Connected to other network - " +
	 * networkType); } else if (state == TelephonyManager.DATA_DISCONNECTED) {
	 * Log.d("PowerDroid-Profiler", "Data connection lost"); } else if (state ==
	 * TelephonyManager.DATA_SUSPENDED) { Log.d("PowerDroid-Profiler",
	 * "Data connection suspended"); }
	 * 
	 * } };
	 * 
	...
	private static class SysClassNet {
		...
		/*
		 * @SuppressWarnings("unused") public static boolean isUp(String inter)
		 * { StringBuilder sb = new StringBuilder();
		 * sb.append(SYS_CLASS_NET).append(inter).append(CARRIER); return new
		 * File(sb.toString()).canRead(); }
		 */
		...
		/**
		 * Added functions to get the number of packets sent and received from
		 * WiFi and 3G<br>
		 * Based on Android 2.2 implementation for TrafficStats<br>
		 * We calculate the received and transmitted packets for 3G reading the
		 * files<br>
		 * "/sys/class/net/rmnet0/statistics/rx_packets" or
		 * "/sys/class/net/ppp0/statistics/rx_packets"<br>
		 * "/sys/class/net/rmnet0/statistics/tx_packets" or
		 * "/sys/class/net/ppp0/statistics/tx_packets"<br>
		 * 
		 * For WiFi interface we read the files<br>
		 * "/sys/class/net/tiwlan0/statistics/rx_packets" and
		 * "/sys/class/net/tiwlan0/statistics/tx_packets"
		 * 
		 * @author Sokol
		 */
	...
	/*
	 * byte timeoutDchFach = 6; // Innactivity timer for transition from DCH ->
	 * FACH byte timeoutFachIdle = 4; // Innactivity timer for transition from
	 * FACH -> IDLE int uplinkThreshold = 151; int downlikThreshold = 119; byte
	 * threegState = THREEG_IN_IDLE_STATE; boolean fromIdleState = true; boolean
	 * fromDchState = false; private long prevRxBytes, prevTxBytes;
	 * 
	 * private void calculate3GStates() { Thread t = new Thread() { public void
	 * run() {
	 * 
	 * while (!stopReadingFiles) {
	 * 
	 * switch(threegState){ case THREEG_IN_IDLE_STATE: threegIdleState(); break;
	 * case THREEG_IN_FACH_STATE: threegFachState(); break; case
	 * THREEG_IN_DCH_STATE: threegDchState(); break; }
	 * 
	 * try {
	 * 
	 * Thread.sleep(Msg.PROFILER_UPDATE_PERIOD);
	 * 
	 * } catch (InterruptedException e) { // e.printStackTrace(); } } } };
	 * t.start(); }
	 * 
	 * private void threegIdleState() { int dataActivity =
	 * telephonyManager.getDataActivity();
	 * 
	 * if( dataActivity == TelephonyManager.DATA_ACTIVITY_IN || dataActivity ==
	 * TelephonyManager.DATA_ACTIVITY_OUT || dataActivity ==
	 * TelephonyManager.DATA_ACTIVITY_INOUT ) { // 3G is in the FACH state
	 * because is sending or receiving data Log.d("PowerDroid-Energy",
	 * "3G in FACH state from IDLE"); threegState = THREEG_IN_FACH_STATE;
	 * fromIdleState = true; threegFachState(); return; }
	 * 
	 * Log.d("PowerDroid-Energy", "3G in IDLE state"); // 3G is in the IDLE
	 * state threeGActiveState.add(THREEG_IN_IDLE_STATE); }
	 * 
	 * private void threegFachState() { if(fromIdleState || fromDchState) { //
	 * The FACH state is just entered from IDLE or DCH, we should stay here at
	 * least 1 second // to measure the size of the buffer and in case to
	 * transit in DCH in the next second fromIdleState = false; fromDchState =
	 * false; prevRxBytes = SysClassNet.getMobileRxBytes(); prevTxBytes =
	 * SysClassNet.getMobileTxBytes(); } else { // 3G was in FACH
	 * if(timeoutFachIdle == 0) { Log.d("PowerDroid-Energy",
	 * "3G in IDLE state from FACH"); timeoutFachIdle = 4; threegState =
	 * THREEG_IN_IDLE_STATE; threegIdleState(); return; } else
	 * if(telephonyManager.getDataActivity() ==
	 * TelephonyManager.DATA_ACTIVITY_NONE) { Log.d("PowerDroid-Energy",
	 * "3G in FACH state with no data activity"); timeoutFachIdle--; } else
	 * timeoutFachIdle = 4;
	 * 
	 * if((SysClassNet.getMobileRxBytes() - prevRxBytes) > downlikThreshold ||
	 * (SysClassNet.getMobileTxBytes() - prevTxBytes) > uplinkThreshold ) {
	 * Log.d("PowerDroid-Energy", "3G in DCH state from FACH"); timeoutFachIdle
	 * = 4; threegState = THREEG_IN_DCH_STATE; threegDchState(); return; } }
	 * 
	 * Log.d("PowerDroid-Energy", "3G in FACH state");
	 * threeGActiveState.add(THREEG_IN_FACH_STATE); }
	 * 
	 * private void threegDchState() { if(timeoutDchFach == 0) {
	 * Log.d("PowerDroid-Energy", "3G in FACH state from DCH"); timeoutDchFach =
	 * 6; threegState = THREEG_IN_FACH_STATE; fromDchState = true;
	 * threegFachState(); return; } else if(telephonyManager.getDataActivity()
	 * == TelephonyManager.DATA_ACTIVITY_NONE) { Log.d("PowerDroid-Energy",
	 * "3G in DCH state with no data activity"); timeoutDchFach--; } else
	 * timeoutDchFach = 6;
	 * 
	...
	/*
	 * public boolean noConnectivity() { return
	...
	/*
	 * public byte get3GActiveState(int i) { return threeGActiveState.get(i); }
	 */
